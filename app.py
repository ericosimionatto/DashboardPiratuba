...
st.set_page_config(
    page_title="23/08/2026 Pedal Piratuba",
    layout="wide"
)
...
# --- TRATAMENTO GLOBAL PARA REMOVER 'None' E 'NaN' ---
# Subtitui valores nulos, a palavra 'None' e 'nan' por texto em branco
df = df.fillna("")
...
with col1:
    if col_cidade and col_estado and col_cidade in df.columns and col_estado in df.columns:
        dados_barras = df.copy()

        dados_barras[col_cidade] = dados_barras[col_cidade].astype(str).str.strip()
        dados_barras[col_estado] = dados_barras[col_estado].astype(str).str.strip().str.upper()
...
with col2:
    ...
    if col_cidade and col_estado and col_cidade in df.columns and col_estado in df.columns:
        ...
        df_mapa_clean[col_cidade] = df_mapa_clean[col_cidade].astype(str).str.strip().str.upper()
        df_mapa_clean[col_estado] = df_mapa_clean[col_estado].astype(str).str.strip().str.upper()
        ...
        df_mapa["Chave"] = (
            df_mapa[col_cidade] + ", " + df_mapa[col_estado]
        )
